...
public class Gardener {
    ...
    public Gardener(InputInfo inputInfo) throws IOException {
        ...
        for (int i = 0; i < flowerbedCount; i++) {
            ...
            flowerbeds.add(new Flowerbed(inputInfo.getShowerTime()[i],inputSensor.getTemperatureSensor().get(0), inputSensor.getWetnessSensor().get(0), 0));
        }
        ...
        while (true) {
            ...
            for (int i=0; i<availableMachines.size(); i++) {
                ...
                if (flowerbed != -1){
                    ...
                    machine.init(timeToMove, flowerbeds.get(flowerbed).getTimeToShower(), flowerbed);
                    machine.dosmth();
                }
                else
                    break;
